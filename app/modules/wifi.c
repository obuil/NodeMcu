...
static uint8 getssidpassword_length=0;
LOCAL struct espconn ptrespconn;


// Description  :  UDP boardcast to phone after successed connected
// add by obuil on 20150609
LOCAL void ICACHE_FLASH_ATTR
connect_callback(void *arg)
{
	 struct espconn *pesp_conn = arg;

	 NODE_ERR("send udp to stop the smart boardcast.\n");
	 // TODO...
	 // don't konw the format of the feedback
    char DeviceBuffer[4] = {'A','C','K','\0'};
    int i=0;
    while(i<5)
    {
    	i++;
    	espconn_sent(pesp_conn, DeviceBuffer, 4);
    }
    espconn_disconnect(pesp_conn);
}

// Description  :  UDP boardcast init
// add by obuil on 20150609
...
user_devicefind_init(void)
{
    ...
    ptrespconn.proto.udp->local_port = 18266;
    ptrespconn.proto.udp->local_ip[0] = 255;
    ptrespconn.proto.udp->local_ip[1] = 255;
    ptrespconn.proto.udp->local_ip[2] = 255;
    ptrespconn.proto.udp->local_ip[3] = 255;
    espconn_regist_connectcb(&ptrespconn, connect_callback);
    espconn_connect(&ptrespconn);
    NODE_ERR("espconn_connect\n");
}


// Description  :  smart config finished
// add by obuil on 20150609
static void smartconfig_done(void *data)
{
	NODE_ERR(" smartconfig_done is called. (%d)\n",data);
	...
	getssidpassword_length = c_strlen(sta_conf->ssid)+c_strlen(sta_conf->password);
	NODE_ERR("smartconfig length(%d)\n",getssidpassword_length);
	wifi_station_set_config(sta_conf);
	wifi_station_disconnect();
	wifi_station_connect();
	user_devicefind_init();

	if(wifi_smart_succeed == LUA_NOREF) return;
	lua_rawgeti(gL, LUA_REGISTRYINDEX, wifi_smart_succeed);
	lua_call(gL, 0, 0);
}
...
static int wifi_start_smart( lua_State* L )
{
	...
	NODE_ERR("wifi_start_smart\n");
	smartconfig_start(type, smartconfig_done);
...
// Description  :  get the current smartconfig status.
// add by obuil on 20150609
static int wifi_smart_status( lua_State* L )
{
	unsigned status;
	status = (unsigned) smartconfig_get_status();
...
/**
  * wifi.setphymode()
  * Description:
  * 	Set wifi physical mode锛�802.11 b/g/n锛�
  * 	Note锛� SoftAP only supports 802.11 b/g.
  * Syntax:
...
/**
  * wifi.getphymode()
  * Description:
  * 	Get wifi physical mode锛�802.11 b/g/n锛�
  * Syntax:
  * 	wifi.getphymode()
  * Parameters:
  * 	nil
...
const LUA_REG_TYPE wifi_map[] = 
{
  ...
  { LSTRKEY( "smartstatus" ), LFUNCVAL( wifi_smart_status ) },
  ...
  // for smartconfig
  { LSTRKEY( "ESPTOUCH" ), LNUMVAL( SC_TYPE_ESPTOUCH ) },
  { LSTRKEY( "AIRKISS" ), LNUMVAL( SC_TYPE_AIRKISS ) },
